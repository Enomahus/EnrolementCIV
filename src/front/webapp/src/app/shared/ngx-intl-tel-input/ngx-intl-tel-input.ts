...
import { CountryCode } from '@app/core/data/country-code';
...
@Component({
  ...
  styleUrls: ['./bootstrap-dropdown.css', './ngx-intl-tel-input.scss'],
  providers: [
    {
...
export class NgxIntlTelInput implements OnInit, OnChanges, ControlValueAccessor {
  ...
  @Output() readonly countryChange = new EventEmitter<CountryItem>();
  ...
  preferredCountriesInDropDown: CountryItem[] = [];
  // Has to be 'any' to prevent a need to install @types/google-libphonenumber by the package user...
  phoneUtil = lpn.PhoneNumberUtil.getInstance();
  disabled = false;
  errors: unknown[] = ['Phone number is required.'];
  countrySearchText = '';
  ...
  registerOnChange(fn: (value: ChangeData | null) => void): void {
  ...
  public searchCountry(): void {
    ...
    const country = this.allCountries.filter((c) => {
      if (this.searchCountryField.indexOf(SearchCountryField.All) > -1) {
        ...
        if (c.codeIso.toLowerCase().startsWith(countrySearchTextLower)) {
          return c;
        }
  ...
  public onPhoneNumberChange(): void {
    ...
    countryCode = countryCode ?? this.selectedCountry.codeIso;
    ...
    if (this.enableAutoCountrySelect) {
      countryCode =
        number && number.getCountryCode()
          ...
          : this.selectedCountry.codeIso;
      if (countryCode && countryCode !== this.selectedCountry.codeIso) {
        const newCountry = this.allCountries
          .slice()
          .sort((a, b) => {
            return a.priority - b.priority;
          })
          .find((c) => c.codeIso === countryCode);
        if (newCountry) {
  ...
  public onCountrySelect(country: CountryItem, el: { focus: () => void }): void {
    this.dropDownOpen = false;
    ...
    if (this.phoneNumber && this.phoneNumber.length > 0) {
      this.value = this.phoneNumber;
      const number = this.getParsedNumber(this.phoneNumber, this.selectedCountry.codeIso);
      const intlNo = number
        ? this.phoneUtil.format(number, lpn.PhoneNumberFormat.INTERNATIONAL)
        : '';
      // parse phoneNumber if separate dial code is needed
      ...
      this.propagateChange({
        ...
        nationalNumber: number ? this.phoneUtil.format(number, lpn.PhoneNumberFormat.NATIONAL) : '',
        e164Number: number ? this.phoneUtil.format(number, lpn.PhoneNumberFormat.E164) : '',
        countryCode: this.selectedCountry.codeIso.toUpperCase(),
        dialCode: '+' + this.selectedCountry.dialCode,
      });
    } else {
      this.propagateChange(null);
  ...
  */
  init(): void {
    ...
    if (this.onlyCountries.length) {
      this.allCountries = this.allCountries.filter((c) => this.onlyCountries.includes(c.codeIso));
    }
  ...
  private removeDialCode(phoneNumber: string): string {
    const number = this.getParsedNumber(phoneNumber, this.selectedCountry.codeIso);
  ...
  private getCountryIsoCode(countryCode: number, number: lpn.PhoneNumber): string | undefined {
    ...
    let matchedCountry = mainCountry ? mainCountry.codeIso : undefined;
    ...
    secondaryCountries.forEach((country) => {
      ...
      country.areaCodes.forEach((areaCode) => {
        if (rawNumber.startsWith(areaCode)) {
          matchedCountry = country.codeIso;
        }
      });
    });
  ...
  protected fetchCountryData(): void {
    ...
    this.countryCodeData.allCountries.forEach((c) => {
      ...
      if (this.enablePlaceholder) {
        country.placeHolder = this.getPhoneNumberPlaceHolder(country.codeIso.toUpperCase());
      }
   ...
   */
  private updatePreferredCountries(): void {
    if (this.preferredCountries.length) {
      this.preferredCountriesInDropDown = [];
      this.preferredCountries.forEach((iso2) => {
        const preferredCountry = this.allCountries.filter((c) => {
          return c.codeIso === iso2;
        });
  ...
  private updateSelectedCountry(): void {
    if (this.selectedCountryISO) {
      this.selectedCountry = this.allCountries.find((c) => {
        return c.codeIso.toLowerCase() === this.selectedCountryISO?.toLowerCase();
      });
